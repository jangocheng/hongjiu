...
namespace Qrcms.Web.admin.article
{
    public partial class article_edit : Web.UI.ManagePage
    {
        ...
        {
            if (rblisupgrade.SelectedValue == "1")
            {
                ...
                ifShow.Visible = true;
                ifShowTwo.Visible = true;
            }
            else if (rblisupgrade.SelectedValue == "0")
            {
                ifShow.Visible = false;
                ifShowTwo.Visible = false;
            }
        }
        #endregion

        #region 绑定大升级================================
        private void TreeBindD(int _is_upgrade)
        {
            BLL.article bll = new BLL.article();
            DataTable dt = bll.GetListDT(_is_upgrade);

            this.ddlCategoryDXJ.Items.Clear();
            this.ddlCategoryDXJ.Items.Add(new ListItem("请选择类别...", ""));
            foreach (DataRow dr in dt.Rows)
            {
                string Id = dr["id"].ToString();
                string Title = dr["title"].ToString().Trim();
                this.ddlCategoryDXJ.Items.Add(new ListItem(Title, Id));
            }
        }
        #endregion
        ...
        private void ShowInfo(int _id)
        {
            ...
            if (rblisupgrade.SelectedValue == "1")
            {
                ...
                ddlCategoryDXJ.SelectedValue = model.big_upgrade.ToString();
                txtzjtkbl.Text = model.zjtkbl.ToString() ;
                ifShow.Visible = true;
                ifShowTwo.Visible = true;
            }
            else if (rblisupgrade.SelectedValue == "0")
        ...
        private bool DoAdd()
        {
            ...
            model.is_upgrade = Utils.StrToInt(rblisupgrade.SelectedValue, 0);
            model.big_upgrade = ddlCategoryDXJ.SelectedValue;
            model.zjtkbl = Utils.ObjToDecimal(txtzjtkbl.Text.Trim(),0);
            //内容摘要提取内容前255个字符
