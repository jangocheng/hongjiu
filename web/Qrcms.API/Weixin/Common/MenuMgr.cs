...
namespace Qrcms.API.Weixin.Common
{
    public class MenuMgr
    {
        ...
        /// 创建菜单
        /// </summary>
        public Senparc.Weixin.Entities.WxJsonResult CreateMenu(string accessToken, Senparc.Weixin.MP.Entities.Menu.ButtonGroup buttonData)
        {
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                var result = Senparc.Weixin.HttpUtility.Post.PostGetJson<Senparc.Weixin.Entities.WxJsonResult>(url, cookieContainer, ms);
                return result;
            }
        }
        ...
        public Senparc.Weixin.MP.Entities.GetMenuResult GetMenu(string accessToken)
        {
            ...
            try
            {
                ...
                {
                    throw new Senparc.Weixin.Exceptions.WeixinException(jsonResult.errmsg);
                }

                finalResult = GetMenuFromJsonResult(jsonResult);
            }
            catch (Senparc.Weixin.Exceptions.WeixinException ex)
            {
                finalResult = null;
            }
        ...
        public Senparc.Weixin.MP.Entities.GetMenuResult GetMenuFromJsonResult(Senparc.Weixin.MP.GetMenuResultFull resultFull)
        {
            ...
            try
            {
                ...
                foreach (var rootButton in resultFull.menu.button)
                {
                    ...
                    if (availableSubButton == 0)
                    {
                        ...
                        {
                            throw new Senparc.Weixin.Exceptions.WeixinMenuException("单击按钮的key不能为空！");
                        }
                ...
                {
                    throw new Senparc.Weixin.Exceptions.WeixinMenuException("一级菜单按钮至少为1个！");
                }

                result = new Senparc.Weixin.MP.Entities.GetMenuResult(bg)
                {
                    menu = bg
                };
            }
            ...
            {
                throw new Senparc.Weixin.Exceptions.WeixinMenuException(ex.Message, ex);
            }
            return result;
        }
