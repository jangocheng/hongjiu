...
{
    public partial class order_yj_list : Web.UI.UsersPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
            ...
            private void RptBind(string _strWhere, string _orderby)
            {
                ...
                string pageUrl = Utils.CombUrlTxt("order_yj_list.aspx", "status={0}&payment_status={1}&express_status={2}&if_upgrade={3}&keywords={4}&page={5}&starttime={6}&endtime={7}&dls={8}&quizresult={9}",
                    this.status.ToString(), this.payment_status.ToString(), this.express_status.ToString(), this.if_upgrade.ToString(), this.keywords, "__id__", this.starttime, this.endtime, this.dlsmz,this.quizresult);
            ...
            strTemp.Append(" and user_name in ( select user_name FROM fn_Getchildlist ('" + admin_info.user_name + "') ) ");
            if (dlsmz.Trim()!="")
            {
                strTemp.Append(" and user_name in ( select user_name FROM fn_Getchildlist ('" + dlsmz.Trim() + "') ) ");
            ...
            protected void Button1_Click(object sender, EventArgs e)
            {
                ...
                if (dt2.Rows.Count > 0)
                {
                    ...
                    //dr2["会员账号"] = "服务费汇总：" + suncovercharge.ToString("0.00") + " 元";
            ...
            {
                Response.Redirect(Utils.CombUrlTxt("order_yj_list.aspx", "status={0}&payment_status={1}&express_status={2}&if_upgrade={3}&keywords={4}&starttime={5}&endtime={6}&dls={7}&quizresult={8}",
              this.status.ToString(), this.payment_status.ToString(), ddlExpressStatus.SelectedValue, this.if_upgrade.ToString(), txtKeywords.Text, txtStartTime.Text, txtEndTime.Text, Txtdls.Text, ddlquizresult.SelectedValue));
        }
...
this.status.ToString(), this.payment_status.ToString(), this.express_status.ToString(), this.if_upgrade.ToString(), txtKeywords.Text, txtStartTime.Text, txtEndTime.Text, Txtdls.Text, ddlquizresult.SelectedValue));
            ...
            protected void btnDelete_Click(object sender, EventArgs e)
            {
                ...
                JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！", Utils.CombUrlTxt("order_yj_list.aspx", "status={0}&payment_status={1}&express_status={2}&if_upgrade={3}&keywords={4}&starttime={5}&endtime={6}&dls={7}&quizresult={8}",
                    this.status.ToString(), this.payment_status.ToString(), this.express_status.ToString(), this.if_upgrade.ToString(), this.keywords, this.starttime, this.endtime, this.dlsmz, ddlquizresult.SelectedValue));
            }
