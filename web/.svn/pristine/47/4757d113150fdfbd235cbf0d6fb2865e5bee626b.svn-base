...
namespace Qrcms.BLL
{
    ...
    public partial class sms_message
    {
        ...
        public bool Send(string mobiles, string content, int pass, out string msg)
        {
            ...
            for (int i = 0; i < batch; i++)
            {
                ...
                if (sb.ToString().Length > 0)
                {
                    try
                    {
                        string result = Utils.HttpPost(siteConfig.smsapiurl,"user=" + siteConfig.smsusername + "&pwd=" + siteConfig.smspassword + "&mobiles=" +  Utils.DelLastComma(sb.ToString()) + "&contents=" + content + "&chid=0&sendtime=");
        ...
        public int GetAccountQuantity(out string code)
        {
            ...
            try
            {
                string result = Utils.HttpPost("http://106008.com/WebAPI/SmsAPI.asmx/GetBalance", "user=" + siteConfig.smsusername + "&pwd=" + siteConfig.smspassword);

                string strEnd = "</Code>";
                string strStart = "<Code>";
                int start = result.IndexOf(strStart);
                result = result.Substring(start + strStart.Length, result.Length - start - strStart.Length);
                int end = result.IndexOf(strEnd);
                if (strEnd.Length == 0 || end < 0)
                {
                    code = "0";
                    return 0;
                }
                string strResult = result.Substring(0, end);
                ...
                code = strResult;
                return Utils.StrToInt(strResult, 0);
            }
            catch
            {
                code = "115";
