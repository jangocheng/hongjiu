...
namespace Qrcms.Web.admin.manager
{
    public partial class manager_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                RptBind("ys=0 and role_type>=" + model.role_type + CombSqlTxt(keywords), "add_time asc,id desc");
            }
        }
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("manager_page_size", "QrcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("manager_page_size", "QrcmsPage", _pagesize.ToString(), 14400);
                }
            }
