...
{
    public partial class auction_list : Web.UI.ManagePage
    {
        ...
        private void RptBind(string _strWhere, string _orderby)
        {
            ...
            this.rptList.DataSource = bll.GetListNew(this.pageSize, this.page, _strWhere, _orderby, out this.totalCount);
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("timesale_page_size", "QrcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            string res = "<a href='auction_comment_list.aspx?no=" + no + "&goodid=" + article_id + "'>查看评论</a>";

            switch (avg_status)
            {
                case "0":
                    res += " | <a href='auction_edit.aspx?no=" + no + "&action=" + DTEnums.ActionEnum.Edit + "'>修改</a>";
                    break;
                case "1":
                case "2":
                    //查询评价价之下之上人数
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("auction_list.aspx", "keywords={0}", txtKeywords.Text));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("timesale_page_size", "QrcmsPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("auction_list.aspx", "keywords={0}", this.keywords));
        }

        //批量删除
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ChkAdminLevel("auction_list", DTEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.auction bll = new BLL.auction();
            ...
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！", Utils.CombUrlTxt("auction_list.aspx", "keywords={0}", this.keywords));
        }
        ...
        /// <returns></returns>
        protected string Getquiz_result(int quiz_result)
        {
            string Type = "";
            if (quiz_result.ToString().IndexOf("1")!=-1)
            {
                Type = "奇";
            }
            else if (quiz_result.ToString().IndexOf("2") != -1)
            {
                Type = "偶";
            }
            return Type;
        ...
        {
            string res = "<a href='auction_order_list.aspx?a_no=" + no + "'>查看订单</a>";
            return res;
        }
