...
namespace Qrcms.DAL
{
    ...
    public partial class orders
    {
        ...
        {
            return Utils.ObjectToStr( DbHelperSQL.GetSingle("select goods_title from dt_order_goods where order_id=" + order_id));
        }
        ...
        public int Add(Model.orders model)
        {
            ...
            SqlParameter[] parameters = {
                    ...
                    new SqlParameter("@is_upgrade",SqlDbType.Int),
                    new SqlParameter("@order_user",SqlDbType.NVarChar,50),
                    new SqlParameter("@performance", SqlDbType.Decimal,5),
					new SqlParameter("@cover_charge", SqlDbType.Decimal,5),
                    new SqlParameter("@order_zjtkbl", SqlDbType.Decimal,2),
                    new SqlParameter("@quiz_time",SqlDbType.DateTime)
                                        };
            ...
            parameters[34].Value = model.is_upgrade;
            parameters[35].Value = model.order_user;
            parameters[36].Value = model.performance;
            parameters[37].Value = model.cover_charge;
            parameters[38].Value = model.order_zjtkbl;
            parameters[39].Value = model.quiz_time;
        ...
        {
            return Utils.ObjToInt(DbHelperSQL.GetSingle("select top 1 article_id from dt_order_goods where order_id =" + order_id + ""), 0);
        }
        ...
        public bool Update(Model.orders model)
        {
            ...
            strSql.Append("order_user=@order_user,");
            strSql.Append("performance=@performance,");
            strSql.Append("cover_charge=@cover_charge,");
            strSql.Append("order_zjtkbl=@order_zjtkbl,quiz_time=@quiz_time");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@order_no", SqlDbType.NVarChar,100),
					new SqlParameter("@trade_no", SqlDbType.NVarChar,100),
					...
					new SqlParameter("@confirm_time", SqlDbType.DateTime),
					new SqlParameter("@complete_time", SqlDbType.DateTime),
                    new SqlParameter("@order_user", SqlDbType.VarChar,50),
                    new SqlParameter("@id", SqlDbType.Int,4),
                    new SqlParameter("@performance", SqlDbType.Decimal,5),
					new SqlParameter("@cover_charge", SqlDbType.Decimal,5),
                    new SqlParameter("@order_zjtkbl", SqlDbType.Decimal,2),
            ...
            parameters[31].Value = model.confirm_time;
            parameters[32].Value = model.complete_time;
            parameters[33].Value = model.order_user;
            parameters[34].Value = model.id;
            parameters[35].Value = model.performance;
            parameters[36].Value = model.cover_charge;
            parameters[37].Value = model.order_zjtkbl;
            parameters[38].Value = model.quiz_time;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id,order_no,trade_no,user_id,user_name,payment_id,payment_fee,payment_status,payment_time,express_id,express_no,express_fee,express_status,express_time,accept_name,post_code,telphone,mobile,email,area,address,message,remark,is_invoice,invoice_title,invoice_taxes,payable_amount,real_amount,order_amount,point,status,add_time,confirm_time,complete_time,is_upgrade,Order_type,quiz,auction_no,quiz_result,order_zjtkbl,quiz_time,cover_charge,performance ");
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" id,order_no,trade_no,user_id,user_name,payment_id,payment_fee,payment_status,payment_time,express_id,express_no,express_fee,express_status,express_time,accept_name,post_code,telphone,mobile,email,area,address,message,remark,is_invoice,invoice_title,invoice_taxes,payable_amount,real_amount,order_amount,point,status,add_time,confirm_time,complete_time,is_upgrade,Order_type,quiz,auction_no,quiz_result,order_zjtkbl,quiz_time");
            strSql.Append(" FROM " + databaseprefix + "orders ");
            if (strWhere.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select *,( select quantity from dt_order_goods where order_id=dt_orders.id) as quantity FROM " + databaseprefix + "orders");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select dt_orders.*,substring([goods_title],1,30)t1,quantity FROM " + databaseprefix + "orders left join [dbo].[dt_order_goods] as b on dt_orders.id=b.order_id");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select dt_orders.order_no as 订单号, (case dt_orders.user_name when '' then '匿名用户' else dt_orders.user_name end) as 会员账号,(case order_type when 1 then '升级' when 2 then '红包' else '' end) as 是否升级,(case status when 1 then (case when payment_status>0 then '待付款' else '待确认' end  ) when 2 then (case when payment_status>1 then '已发货' else (case when accept_name='' then '已付款' else '待发货' end) end  ) when 3 then '交易完成' when 4 then '已取消' when 5 then '已作废' end ) as 订单状态,order_amount as 总金额,performance as 业绩, add_time as 下单时间, substring([goods_title],1,30) as 商品标题,quantity as 数量 FROM " + databaseprefix + "orders left join [dbo].[dt_order_goods] as b on dt_orders.id=b.order_id");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"select  a.auction_no,a.order_amount,a.cover_charge,a.performance,a.order_user,a.id,[order_no],[user_name],[status],[quantity],[real_price],[payment_time],[quiz_result],[img_url],[goods_title], substring([goods_title],1,30)t1,[goods_id],quiz_time,order_zjtkbl ");
            strSql.Append(" from [dbo].[dt_orders] as a left join [dbo].[dt_order_goods] as b on a.id=b.order_id");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        public Model.orders DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                if (row["cover_charge"] != null)
                {
                    model.cover_charge =Convert.ToDecimal( row["cover_charge"].ToString());
                }
                if (row["performance"] != null)
                {
                    model.performance = Convert.ToDecimal(row["performance"].ToString());
                }
                ...
                if (row["order_zjtkbl"] != null && row["order_zjtkbl"].ToString() != "")
                {
                    model.order_zjtkbl = decimal.Parse(row["order_zjtkbl"].ToString());
                }
                if (row["quiz_time"] != null && row["quiz_time"].ToString() != "")
                {
                    model.quiz_time = DateTime.Parse(row["quiz_time"].ToString());
                }
                //,is_upgrade,Order_type,quiz,auction_no,quiz_result
                model.is_upgrade = Utils.ObjToInt(row["is_upgrade"]);
                model.Order_type = Utils.ObjToInt(row["order_type"]);
                model.quiz = Utils.ObjToInt(row["quiz"]);
                model.auction_no = Utils.ObjectToStr(row["auction_no"]);
                model.quiz_result = Utils.ObjToInt(row["quiz_result"]);
                #endregion
