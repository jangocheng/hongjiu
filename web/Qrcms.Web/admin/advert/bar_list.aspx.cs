...
{
    public partial class bar_list : Qrcms.Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("advert_list", DTEnums.ActionEnum.View.ToString()); //检查权限
                TreeBind(); //绑定广告位
        ...
        {
            ChkAdminLevel("advert_bar", DTEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.advert_banner bll = new BLL.advert_banner();
            ...
            JscriptMsg("保存排序成功！", Utils.CombUrlTxt("bar_list.aspx", "aid={0}&keywords={1}", this.aid.ToString(), this.keywords), "parent.loadMenuTree");
        }

        //批量删除
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ChkAdminLevel("advert_bar", DTEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.advert_banner bll = new BLL.advert_banner();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！",
                Utils.CombUrlTxt("bar_list.aspx", "aid={0}&keywords={1}", this.aid.ToString(), this.keywords), "parent.loadMenuTree");
        }
