...
namespace Qrcms.DAL
{
	...
	public partial class user_recharge
	{
        ...
        /// 确认充值订单
        /// </summary>
        public bool Confirm(string recharge_no, Model.siteconfig configModel)
        {
            ...
            decimal amount = 0;
            decimal sxf = Utils.StrToDecimal((model.amount * configModel.czsxfbl).ToString("0.00"),0);
            amount = model.amount - sxf;
            //增加一条账户余额记录
            StringBuilder strSql3 = new StringBuilder();
            strSql3.Append("insert into " + databaseprefix + "user_amount_log(");
            strSql3.Append("user_id,user_name,value,remark,add_time)");
            ...
            parameters3[1].Value = model.user_name;
            parameters3[2].Value = model.amount;
            parameters3[3].Value = "在线充值，单号：" + recharge_no+"，到账："+amount+"，手续费："+sxf;
            parameters3[4].Value = DateTime.Now;
            List<CommandInfo> sqllist = new List<CommandInfo>();
            CommandInfo cmd = new CommandInfo(strSql3.ToString(), parameters3);
            sqllist.Add(cmd);
            //修改用户表金额
            StringBuilder strSql2 = new StringBuilder();
            strSql2.Append("update " + databaseprefix + "users set amount=amount+" + amount);
            strSql2.Append(" where id=@id");
            SqlParameter[] parameters2 = {
                    new SqlParameter("@id", SqlDbType.Int,4)};
            parameters2[0].Value = model.user_id;
