...
{
    public partial class auction_loading : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("auction_loading", DTEnums.ActionEnum.View.ToString()); //检查权限
        ...
        private void RptBind(string _strWhere, string _orderby)
        {
            ...
            string pageUrl = Utils.CombUrlTxt("auction_loading.aspx", "keywords={0}&page={1}", this.keywords, "__id__");
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("auction_loading_page_size", "QrcmsPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("auction_loading.aspx", "keywords={0}", txtKeywords.Text));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("auction_loading_page_size", "QrcmsPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("auction_loading.aspx", "keywords={0}", this.keywords));
        }

        //批量删除
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ChkAdminLevel("auction_loading", DTEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.auction bll = new BLL.auction();
            ...
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！", Utils.CombUrlTxt("auction_loading.aspx", "keywords={0}", this.keywords));
        }
        ...
        {
            string res = "<a href='auction_order_list.aspx?a_no=" + no + "'>查看</a>";
            return res;
        }
    }
}