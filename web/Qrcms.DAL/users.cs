...
namespace Qrcms.DAL
{
    ...
    public partial class users
    {
        ...
        /// 根据用户名返回一个实体
        /// </summary>
        public Model.users GetModelopenid(string openid)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        public int Add(Model.users model)
        {
            ...
            strSql.Append("group_id,user_name,salt,password,mobile,email,avatar,nick_name,sex,birthday,telphone,area,address,qq,msn,amount,point,exp,status,reg_time,reg_ip,QRcodestate,real_name,idcard,commision,parent_user)");
            ...
            SqlParameter[] parameters = {
                    ...
                    new SqlParameter("@QRcodestate", SqlDbType.NVarChar,20),
                    new SqlParameter("@real_name", SqlDbType.NVarChar,500),
                    new SqlParameter("@idcard", SqlDbType.NVarChar,500),
                    new SqlParameter("@commision", SqlDbType.Decimal,5),
                    new SqlParameter("@parent_user",SqlDbType.VarChar,50)
                                        };
            ...
            parameters[21].Value = model.QRcodestate;
            parameters[22].Value = model.real_name;
            parameters[23].Value = model.idcard;
            parameters[24].Value = model.commision;
            parameters[25].Value = model.parent_user;
        ...
        {
            return Utils.ObjectToStr(DbHelperSQL.GetSingle("select user_name from dt_users where id=" + id));
        }
        ...
        public bool Update(Model.users model)
        {
            ...
            strSql.Append("QRcodestate=@QRcodestate,");
            strSql.Append("real_name=@real_name,");
            strSql.Append("idcard=@idcard,");
            strSql.Append("commision=@commision,parent_user=@parent_user");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@group_id", SqlDbType.Int,4),
					new SqlParameter("@user_name", SqlDbType.NVarChar,100),
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id,group_id,user_name,salt,password,mobile,email,avatar,nick_name,sex,birthday,telphone,area,address,qq,msn,amount,point,exp,status,reg_time,reg_ip,QRcodestate,real_name,idcard,commision,parent_user,openid");
            strSql.Append(" from " + databaseprefix + "users");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
        ...
        public Model.users GetModel(string user_name, string password)
        {
            ...
            strSql.Append(" where user_name=@user_name and password=@password and status=0");
            SqlParameter[] parameters = {
					new SqlParameter("@user_name", SqlDbType.NVarChar,100),
                    new SqlParameter("@password", SqlDbType.NVarChar,100)};
            parameters[0].Value = user_name;
        ...
        /// <returns></returns>
        public object GetList(string select ,string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" id,group_id,user_name,salt,password,mobile,email,avatar,nick_name,sex,birthday,telphone,area,address,qq,msn,amount,point,exp,status,reg_time,reg_ip,QRcodestate,real_name,idcard,commision,parent_user,openid");
            strSql.Append(" FROM " + databaseprefix + "users ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
        ...
        /// 获得查询分页数据（我的下级0)
        /// </summary>
        public DataSet GetList(int pageSize, int pageIndex, string strWhere, string filedOrder, out int recordCount,string username)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select fn_Getchildlist.*,(select title from dt_user_groups where id=dt_users.group_id)as jb,email,amount,point,status,QRcodestate,mobile,nick_name,reg_time FROM fn_Getchildlist ('" + username + "') left join dt_users on fn_Getchildlist.user_name=dt_users.user_name ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        public Model.users DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                if (row["QRcodestate"] != null)
                {
                    model.QRcodestate = row["QRcodestate"].ToString();
                }
                ...
                if (row["commision"] != null)
                {
                    model.commision = Utils.StrToDecimal(row["commision"].ToString(),0);
                }
                if (row["parent_user"] != null)
                {
                    model.parent_user = row["parent_user"].ToString(); 
                }
                model.openid = Utils.ObjectToStr(row["openid"]);
            }
            return model;
        }
        #endregion
