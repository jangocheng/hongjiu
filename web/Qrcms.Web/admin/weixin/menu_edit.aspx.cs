...
namespace Qrcms.Web.admin.weixin
{
    public partial class menu_edit : Web.UI.ManagePage
    {
        ...
        private void GetMenu(int account_id)
        {
            ...
            string sql = "select * from Qr_wxcaidan where caidan_layid=0";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    txtName.Text = dr["caidan_name"].ToString();
                    if (dr["caidan_type"].ToString().Trim() == "view")
                    {

                        txtUrl.Text = dr["caidan_url"].ToString().Trim();
                    }
                    else
                    {
                        txtKey.Text = dr["caidan_key"].ToString().Trim();

                    }
                    getxiaji(dr["caidan_cid"].ToString().Trim(), i);
                }
        ...
        #endregion
        private void getxiaji(string id, int i)
        {
            string sql = "select * from Qr_wxcaidan where caidan_layid=" + id + "";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int j = 0; j < dt.Rows.Count; j++)
                {
                    ...
                    txtSubName.Text = dr["caidan_name"].ToString();
                    if (dr["caidan_type"].ToString().Trim() == "view")
                    {

                        txtSubUrl.Text = dr["caidan_url"].ToString().Trim();
                    }
                    else
                    {
                        txtSubKey.Text = dr["caidan_key"].ToString().Trim();
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DbHelperSQL.ExecuteSql("delete from [Qr_wxcaidan]");
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    string strSql = "INSERT INTO [Qr_wxcaidan]([caidan_name],[caidan_cid],[caidan_type] ,[caidan_key] ,[caidan_retype] ,[caidan_retext] ,[caidan_url] ,[caidan_layid] ,[caidan_order] ,[caidan_addtime]) VALUES ('" + txtName.Text.Trim() + "','" + (i + 1) + "','view','" + txtKey.Text.Trim() + "','','','" + txtUrl.Text.Trim() + "','0'," + i + ",getdate())";
                    ...
                    for (int j = 0; j < 5; j++)
                    {
                        ...
                        strSql = "INSERT INTO [Qr_wxcaidan]([caidan_name],[caidan_cid],[caidan_type] ,[caidan_key] ,[caidan_retype] ,[caidan_retext] ,[caidan_url] ,[caidan_layid] ,[caidan_order] ,[caidan_addtime]) VALUES ('" + txtSubName.Text.Trim() + "','" + (i + 1) + (j + 1) + "','view','" + txtSubKey.Text.Trim() + "','','','" + txtSubUrl.Text.Trim() + "','" + (i + 1) + "'," + j + ",getdate())";
                        DbHelperSQL.ExecuteSql(strSql);
